  ...
  return (
    <div className="w-full max-w-sm mx-auto">
      ...
      <p className="text-sm text-charcoal/45 mb-10">
        Log in to your ClassDeck account.
      </p>
      ...
      <form className="space-y-5" onSubmit={handleEmailLogin}>
        ...
        <div className="flex items-center justify-between">
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              type="checkbox"
              className="w-3.5 h-3.5 rounded border-border text-sienna focus:ring-sienna/20"
            />
            <span className="text-xs text-charcoal/50">Remember me</span>
          </label>
          <Link
            href="/forgot-password"
            className="text-xs text-sienna hover:text-sienna-dark transition-colors"
          >
            Forgot password?
          </Link>
        </div>
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full bg-charcoal text-white text-sm font-medium py-3 rounded-xl hover:bg-charcoal-light transition-all duration-300 cubic-bezier(0.25, 0.8, 0.25, 1) hover:shadow-lg hover:shadow-charcoal/10 disabled:opacity-50 flex items-center justify-center gap-2"
        >
          {loading ? (
            <>
